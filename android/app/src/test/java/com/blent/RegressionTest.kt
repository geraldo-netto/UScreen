...
class RegressionTest {
    ...
    @Test fun t250_forkPackageKeepsProtectedOriginalClasses() {
        assertEquals("io.github.geraldo_netto.blent", app.packageName)
        for (name in listOf("MainActivity", "TokenActivity")) {
            val component = ComponentName(app.packageName, "com.blent.$name")
            val info = app.packageManager.getActivityInfo(component, 0)
            assertEquals("com.blent.$name", info.name)
        }
        val receiver = ComponentName(app.packageName, "com.blent.TokenReceiver")
    ...
    @Test fun t039_tokenDeliveryRequiresShellPermissionAndFrontsLauncher() {
        val component = ComponentName(app.packageName, "com.blent.TokenActivity")
        val info = app.packageManager.getActivityInfo(component, 0)
