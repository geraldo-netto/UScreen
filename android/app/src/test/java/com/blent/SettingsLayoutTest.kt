...
class SettingsLayoutTest {
    ...
    @Test fun t377_recompositionAndLocalSettingsPreserveReceiverOwnership() {
        ...
        compose.setContent { BlentTheme {
            BlentMain({}, presentation = session.presentation, settings = session.settings,
    ...
    @Test fun t388_battery_toggle_is_opt_in_persistent_and_preserves_display_and_stream() {
        ...
        val prefs = Prefs(app)
        val saved = app.getSharedPreferences("blent", android.content.Context.MODE_PRIVATE)
    ...
    @Test fun t388_hidden_stats_do_not_schedule_presentation_samples() {
        ...
        compose.setContent { BlentTheme {
            BlentMain({}, presentation = presentation, settings = SettingsValues(showStats = visible.value))
        } }
    ...
    private fun checkDisplayControls(penOnly: Boolean) {
        ...
        val session = SessionCoordinator(prefs, { it() }, null, null)
        compose.setContent { BlentTheme {
            BlentMain({}, penOnly = penOnly, settings = session.settings,
    ...
    private fun checkWebLinks(browserAvailable: Boolean, vararg labels: String) {
        ...
        var dismissed = false
        compose.setContent { BlentTheme {
            BlentMain({}, updateAvailable = "9.9.9", showThanks = true,
    ...
    @Test fun t241_t247_penModeRequiresAuthenticatedControlAndRecoversWithoutVideo() {
        ...
        compose.setContent { BlentTheme { BlentMain({}, penOnly = true, presentation = presentation) } }
        try {
