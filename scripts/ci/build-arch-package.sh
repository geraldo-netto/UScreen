...
BUILD=$(mktemp -d)
chmod 755 "$BUILD"
cp /source/packaging/arch/{PKGBUILD,blent.install} "$BUILD/"
mkdir -p "$BUILD/src"
tar -xf "/artifacts/blent-$VERSION-linux-x86_64.tar.gz" -C "$BUILD/src"
mv "$BUILD/src/blent-$VERSION" "$BUILD/src/UScreen-$VERSION"
...
runuser -u packager -- bash -c '
    ...
    cd "$1"
    source PKGBUILD
    srcdir="$PWD/src"; pkgdir="$PWD/pkg/blent"
    mkdir -p "$pkgdir"
    cd "$srcdir"
    package
    cd "$1"
...
' blent-arch "$BUILD"
cp "$BUILD"/blent-[0-9]*.pkg.tar.zst /artifacts/
