...
def generate(output):
    ...
    command = ['ffmpeg', '-v', 'error', '-f', 'lavfi', '-i', 'testsrc2=size=1280x720:rate=30',
               ...
               '-tune', 'zerolatency', '-preset', 'ultrafast', '-b:v', '3000k', '-maxrate', '3000k',
               '-bufsize', '3000k', '-g', '30', '-keyint_min', '30', '-sc_threshold', '0',
    ...
    metadata = dict(command=command, ffmpeg=subprocess.check_output(['ffmpeg', '-version'], text=True),
                    sha256=hashlib.sha256(data).hexdigest(), frames=len(packets), width=1280, height=720, fps=30)
