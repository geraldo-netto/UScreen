...
impl Monitor {
    pub(super) fn check_forwarding(&mut self) {
        let routes: Vec<_> = self
            .ready
            .iter()
            .filter_map(|serial| self.forwarding_ports(serial).map(|ports| (serial.clone(), ports)))
            .collect();
    ...
    fn forwarding_ports(&self, serial: &str) -> Option<(u16, u16)> {
        ...
        self.extras.get(serial).map(|session| (session.video_port, session.input_port))
    }
}
...
async fn repair(adb: &str, serial: &str, ports: (u16, u16)) -> Result<()> {
    ...
    for (remote, local) in config::adb_reverse::missing(text, &expected)? {
        let result = tokio::process::Command::new(adb)
            .args(["-s", serial, "reverse", "--no-rebind", &format!("tcp:{remote}"), &format!("tcp:{local}")])
            .output_bounded()
            .await?;
        anyhow::ensure!(result.status.success(), "Could not restore missing ADB reverse tcp:{remote}");
...
"#).unwrap();
    ...
    async fn t557_repairs_only_missing_routes_and_uses_no_rebind() {
        ...
        repair(adb.to_str().unwrap(), "USB", (9010, 9011)).await.unwrap();
        assert_eq!(std::fs::read_to_string(adb.with_extension("args")).unwrap(),
            "-s USB reverse --list\n-s USB reverse --no-rebind tcp:8891 tcp:9011\n");
        std::fs::write(adb.with_extension("listing"), "UsbFfs tcp:8890 tcp:9010\nUsbFfs tcp:8891 tcp:9011\n").unwrap();
        std::fs::remove_file(adb.with_extension("args")).unwrap();
        repair(adb.to_str().unwrap(), "USB", (9010, 9011)).await.unwrap();
        assert_eq!(std::fs::read_to_string(adb.with_extension("args")).unwrap(), "-s USB reverse --list\n");
    }

    #[tokio::test]
    async fn t557_conflicts_malformed_lists_and_backend_failures_never_rebind() {
        let root = tempfile::tempdir().unwrap();
        for listing in [b"UsbFfs tcp:8891 tcp:2222\n".as_slice(), b"truncated", b"\xff"] {
            let adb = fake_adb(root.path(), listing);
            std::fs::remove_file(adb.with_extension("args")).ok();
            assert!(repair(adb.to_str().unwrap(), "USB", (9010, 9011)).await.is_err());
            assert_eq!(std::fs::read_to_string(adb.with_extension("args")).unwrap(), "-s USB reverse --list\n");
        }
        ...
        assert!(repair("/missing-t557-adb", "USB", (9010, 9011)).await.is_err());
    }
    ...
    async fn t557_periodic_repair_preserves_assignment_and_joins_existing_mutations() {
        ...
        state.mutations.schedule("USB".into(), std::future::pending());
    ...
    async fn t557_extra_repair_uses_its_existing_slot_ports() {
        ...
        let session = session::Spec {
            capture: capture::CaptureConfig { instance: 1, helper_path: "/missing-t557-helper".into(), ..Default::default() },
            ports: (0, 0), token: None, devices: (false, false, false),
        }.prepare(state.config.extra.mode_tx.clone()).start(state.config.extra.shutdown_rx.clone()).await.unwrap();
