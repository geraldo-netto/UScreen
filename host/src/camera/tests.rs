...
print('Broadcast completed: result=1')"#,
    ...
    assert_eq!(command, format!("am broadcast -n io.github.geraldo_netto.blent/com.blent.CameraReceiver --es token {token} --ei port 34567 --ei width 160 --ei height 120 --ei fps {} --ei lens 0 --ez background false --ei bitrate {} --ei freshness_ms {} --ez adaptive_bitrate {} --ei min_bitrate {}\n", options().fps, options().bitrate, options().freshness_ms, options().adaptive_bitrate, options().min_bitrate));
}
