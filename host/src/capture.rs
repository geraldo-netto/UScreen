...
impl CaptureManager {
    ...
    async fn run_encoder_session(
        ...
        #[cfg(feature = "inproc-encoder")]
        let _stop_on_cancel = StopEncoderOnDrop(stop_encode.clone());
...
// A spawn_blocking task cannot be aborted. Retire its reader even when the
// session future is dropped before reaching an explicit shutdown branch.
#[cfg(feature = "inproc-encoder")]
struct StopEncoderOnDrop(Arc<std::sync::atomic::AtomicBool>);

#[cfg(feature = "inproc-encoder")]
impl Drop for StopEncoderOnDrop {
    fn drop(&mut self) {
...
mod inproc_tests {
    ...
    async fn cancel_waiting_encoder() -> (bool, bool) {
        ...
        let mut run = CaptureRun {
            ...
            display_rx,
            shutdown_rx,
            mode_rx: manager.mode_rx.clone(),
            stream_rx: manager.stream_rx.clone(),
            backoff_ms: 0,
            explained_evdi: false,
            pipeline_started_at: Instant::now(),
