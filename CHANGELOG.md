...
- Rename the project to Blent with new application identities and fresh settings;
  retain original copyright/license notices. No old-version migration or API
...
- Each device can be switched off: `input_touch`, `input_pen`,
  ...
  leaving the tablet blank. `blent doctor` reports which devices are on.
...
- Fix: the virtual monitor no longer exists while no tablet is attached. The
  ...
  Blent screen" came up with the real screens black and no tablet in sight.
...
- Packaging: the `.deb` recommends `evdi-dkms` instead of depending on it, so
  ...
  `blent` unconfigured with its udev rule and modprobe file missing;
...
- Fix: with two Android devices attached the daemon took whichever one adb
  ...
  the Blent app installed (#10 (historical upstream)).
...
- Fix: the pen's tilt axes were the wrong way round — a pen leaning right
  ...
  (#11 (historical upstream)).
- `blent doctor` tells two Android devices apart from one device reachable
...
- `blent wifi` sets the tablet up for wireless use in one step — it switches
  ...
  repeat by hand (#8 (historical upstream)).
  `blent wifi --off` forgets it. The video and input ports stay on loopback.
- Fix: on Debian- and Ubuntu-based Plasma systems the daemon never mapped the
  ...
  (#9 (historical upstream),
  #10 (historical upstream)). The same call path
  carries on-screen-keyboard suppression, which was silently off there too.
- `blent doctor` reports whether KWin can be reached at all, instead of
  leaving the Desktop section empty when it cannot.
- The settings window now identifies itself to the desktop as `blent`, the
  same name as its menu entry, so the KDE task bar shows the Blent icon for
...
- Icons: the app, the menu entry, the settings window and the tray now share
  one Blent icon (amber tablet and stylus on charcoal) instead of the stock
  Android tile and a generic display glyph; the tray shows a dimmed variant
  in graphics-tablet mode.
- `blent doctor` warns when Samsung's Motion smoothness is on Standard,
...
- App: *Rotate automatically* in the ⚙ sheet follows the tilt sensor between
  ...
  discussion #7 (historical upstream)); with
...
- Fix: after leaving graphics-tablet mode the pen and touch could stay mapped
  ...
  assuming it (#6 (historical upstream)).
...
- Fixes: PID-file race between daemon restarts, `blent stop` matching
...
- `blent doctor`, end-to-end latency measurement, input mapped to the
