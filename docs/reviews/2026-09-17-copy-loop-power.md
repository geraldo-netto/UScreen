...
| --- | --- | --- |
| T383 | `mark_damage` revisits chroma rows for overlapping rectangles; conversion dispatch wakes every worker and checks every assigned row. | Dirty-range/word operations and nonempty jobs versus current scalar kernels, for sparse/full damage and scales 1–4; preserve exact rounding and the histories of all three buffers. |
...
| T404 | Android performs two reverse searches through 64 timing slots; host ACK handling searches a 256-entry deque and periodically discards sample-vector capacity. | Tagged direct lookup and safe reuse only if profiling justifies the extra state; test wrap, old generations, callback order and concurrent publication. |
| T405 | FIFO input uses 2/5 ms sleeps; server shutdown/config checks poll at 100 ms; C capture waits are capped at 4 ms and writer waits expire each frame period. | Readiness/cancellation events and precise work/keepalive deadlines; preserve stop/reopen, partial-frame and mode-change behavior without HUP spin or missed wakeups. |
| T406 | `UInputDevice::emit` performs one write per native event. | Batch within each existing SYN_REPORT boundary, with no sample delay and identical pen/touch lifecycle frames. |
