...
| T390: discovery fairness | At review time identity/package probes used unbounded concurrent batches with a whole-batch barrier; setup and recovery were sequential. T390 replaces the monitor with bounded independently published results; see the [discovery replay](../benchmarks/2026-09-17-discovery.md). | Use fake ADB devices with deterministic delays/failures, then measure unaffected-device recovery. Consider bounded per-device concurrency while serializing shared forwarding operations and preserving stable identities/cancellation. |
