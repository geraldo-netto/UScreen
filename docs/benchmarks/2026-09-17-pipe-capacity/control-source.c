...
struct session {
    int reader, writer, capacity, frames, paced, delay_ms, sample_pending;
    size_t size;
...
static void send_frame(struct session *session) {
    ...
    while (offset < session->size) {
        ...
        assert(count > 0);
        offset += (size_t)count;
        if (session->sample_pending) sample_pending(session);
    }
}
...
static void initialize(struct session *session, pthread_barrier_t *barrier, char **argv, int sample) {
    int pipe[2];
    assert(pipe2(pipe, O_CLOEXEC) == 0);
    session->reader = pipe[0]; session->writer = pipe[1]; session->start = barrier;
    session->sample_pending = sample;
...
int main(int argc, char **argv) {
    ...
    assert(sessions);
    int sample = argc == 7 ? 1 : atoi(argv[7]);
    for (int i = 0; i < count; i++) initialize(&sessions[i], &barrier, argv, sample);
