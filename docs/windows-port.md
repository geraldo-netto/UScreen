...
| Unix signals, `/proc`, UID checks and file permissions | `host/src/main.rs`, `common/src/lib.rs`, `common/src/runtime.rs`, `host/src/runtime.rs`, `host/src/stream.rs` | Windows process handles/identity, controlled shutdown, per-user single-instance handling, private paths/ACLs, secure randomness and portable socket handling. |
...
| VAAPI defaults and Linux build/packaging | `common/src/lib.rs`, `host/src/capture.rs`, `host/src/encoder.rs`, `Makefile`, `scripts/`, `packaging/`, `.github/workflows/` | Windows encoder capability detection, build scripts, tests and installation artifacts. |
